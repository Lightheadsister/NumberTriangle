...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        char first_char = path.charAt(0);
        String remain = path.substring(1);

        if (first_char == ('l')) {
            return left.retrieve(remain);
        } else if (first_char == 'r') {
            return right.retrieve(remain);
        }
        return -1;
    }
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            for(String num : numbers){
                current.add(new NumberTriangle(Integer.parseInt(num)));
            }

            if(!tree.isEmpty()){
                List<NumberTriangle> prev = tree.get(tree.size() - 1);
